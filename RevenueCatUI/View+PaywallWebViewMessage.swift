...
extension View {
    ...
    ///                 variables: ["custom": .object(["app_segment": .string("high_intent")])]
    ///             )
    ///
    ///         case "rc:error":
    ...
    /// ```
    ///
    /// - Note: For `"rc:request-variables"`, the SDK automatically replies with SDK-managed
    ///   variables (`locale`, `color_scheme`) and the paywall's custom variables before invoking
    ///   this handler, so you only need to send any *additional* variables.

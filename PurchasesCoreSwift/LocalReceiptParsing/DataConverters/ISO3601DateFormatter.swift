...
extension DateFormatter {
    ...
    static let iso8601MiliSecondsDateFormatter: DateFormatter = {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
        return dateFormatter
    }()

    func iso8601Date(fromBytes bytes: ArraySlice<UInt8>) -> Date? {
        ...
        return (Self.iso8601SecondsDateFormatter.date(from: dateString)
            ?? Self.iso8601MiliSecondsDateFormatter.date(from: dateString))
    }

    func iso8601Date(fromString maybeDateString: String?) -> Date? {

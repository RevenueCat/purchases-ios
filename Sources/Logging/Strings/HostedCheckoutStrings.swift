...
enum HostedCheckoutStrings {
    ...
    case poll_failed(_ operationSessionID: String, failure: HostedCheckoutStatusResponse.Failure?)
...
extension HostedCheckoutStrings: LogMessage {
    ...
    var description: String {
        ...
        case let .poll_failed(operationSessionID, failure):
            let detail = failure.map { "code \($0.code) (\($0.message ?? "no message"))" } ?? "no detail"

...
extension RulesEngine {
    ...
    enum EqualityOperators {
        ...
        static func opLooseEq(args: Value, vars: Scope) throws -> Value {
            let (lhs, rhs) = try Operators.evalTwo(args, vars: vars)
            return .bool(looseEq(lhs, rhs))
        }

        /// `{"!=": [a, b]}` — JSON Logic loose inequality. Negation of `==`.
        static func opLooseNe(args: Value, vars: Scope) throws -> Value {
            let (lhs, rhs) = try Operators.evalTwo(args, vars: vars)
            return .bool(!looseEq(lhs, rhs))
        }
        ...
        /// value (`1 === "1"` is false).
        static func opStrictEq(args: Value, vars: Scope) throws -> Value {
            let (lhs, rhs) = try Operators.evalTwo(args, vars: vars)
            return .bool(strictEq(lhs, rhs))
        }

        /// `{"!==": [a, b]}` — JSON Logic strict inequality. Negation of `===`.
        static func opStrictNe(args: Value, vars: Scope) throws -> Value {
            let (lhs, rhs) = try Operators.evalTwo(args, vars: vars)
            return .bool(!strictEq(lhs, rhs))
        }
    }

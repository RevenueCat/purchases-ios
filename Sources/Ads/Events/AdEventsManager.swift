...
actor AdEventsManager: AdEventsManagerType {
    ...
    func track(featureEvent: FeatureEvent) async {
        guard let event: StoredEvent = .init(event: featureEvent,
            ...
            Logger.error(Strings.ads.event_cannot_serialize)
            return
        }
    ...
    func flushEvents(count: Int) async throws -> Int {
        guard !self.flushInProgress else {
            Logger.debug(Strings.ads.event_flush_already_in_progress)
            return 0
        }
        ...
        guard !events.isEmpty else {
            Logger.verbose(Strings.ads.event_flush_with_empty_store)
            return 0
        }

        Logger.verbose(Strings.ads.event_flush_starting(count: events.count))
        ...
        do {
            ...
            Logger.error(Strings.ads.event_sync_failed(error))

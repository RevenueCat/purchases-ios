...
struct ProductRequestData {
    ...
    var cacheKey: String {
        var key =
        """
        \(productIdentifier)-\(price)-\(currencyCode ?? "")-\(storefront ?? "")-\(paymentMode?.rawValue ?? -1)-\
        \(introPrice ?? 0)-\(subscriptionGroup ?? "")-\(normalDuration ?? "")-\(introDuration ?? "")-\
        \(introDurationType?.rawValue ?? -1)
        """
...
extension ProductRequestData: Encodable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(self.storefront, forKey: .storefront)

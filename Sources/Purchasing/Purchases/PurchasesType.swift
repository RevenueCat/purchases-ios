...
// There is a comment with a long URL that is causing the line length to be exceeded.
...
public protocol PurchasesType: AnyObject {
    ...
    var delegate: PurchasesDelegate? { get set }
         ...
         *
         * - Parameter appUserID: The ``appUserID`` that should be linked to the current user.
         * - Parameter attributes: Subscriber attributes to set on the user being logged in.
         * Attributes that couldn't be set don't fail the log in: they're logged and retried on the next
         * attribute sync.
         *
         * The `completion` block will be called with the latest ``CustomerInfo`` and a `Bool` specifying
         * whether the user was created for the first time in the RevenueCat backend.
    ...
    func purchase(package: Package) async throws -> PurchaseResultData
         ...
         * If the user cancelled the purchase, `userCancelled` will be `true`.
         */
        func purchase(product: StoreProduct, promotionalOffer: PromotionalOffer) async throws
            -> PurchaseResultData
         ...
         */
        func checkTrialOrIntroDiscountEligibility(productIdentifiers: [String]) async -> [String:
            IntroEligibility]
        ...
        func beginRefundRequest(forEntitlement entitlementID: String) async throws
            -> RefundRequestStatus
    ...
    func presentCodeRedemptionSheet()
         ...
         */
        @objc func syncAttributesAndOfferingsIfNeeded(
        ...
        func purchaseWithParams(
            _ params: PurchaseParams, completion: @escaping PurchaseCompletedBlock)

...
@available(iOS 15.0, *)
@MainActor
final class RewardedInterstitialPresentationTests: AdapterTestCase {

    private static let adUnitID = "ca-app-pub-test/rewarded-interstitial"

    func testDismissalWithoutEarningRewardCompletesWithShown() throws {
        let fakeAd = FakeRewardedInterstitialAd()
    ...
    func testMissingPresentationContextCompletesWithFailedWithoutPresenting() throws {
        ...
        let error = try XCTUnwrap(recorder.outcomes.first?.error)
        XCTAssertEqual(error.domain, RewardedInterstitialPresentationError.errorDomain)
        XCTAssertEqual(error.code, RewardedInterstitialPresentationError.noPresentationContext.errorCode)
        XCTAssertTrue(fakeAd.events.isEmpty)
    }

    func testUnsupportedMediatorCompletesWithFailedWithoutLoading() throws {
        var loadCount = 0
        let presentation = RewardedInterstitialPresentation(
            loadAd: { _, _, _ in
                loadCount += 1
                return FakeRewardedInterstitialAd()
            },
        ...
        XCTAssertEqual(
            error.code,
            RewardedInterstitialPresentationError.unsupportedMediator(MediatorName.appLovin.rawValue).errorCode
        )
    ...
    func testOutcomeMapsToAdPresentationResult() {
        XCTAssertTrue(
            RewardedInterstitialPresentation.Outcome.shown.presentationResult === AdPresentationResult.shown
        )
        XCTAssertTrue(
            RewardedInterstitialPresentation.Outcome.rewardVerificationFailed.presentationResult
                === AdPresentationResult.rewardVerificationFailed
        )

        let rewarded = RewardedInterstitialPresentation.Outcome.rewarded(.noReward, moreRewards: []).presentationResult
        XCTAssertFalse(rewarded === AdPresentationResult.shown)
        XCTAssertFalse(rewarded === AdPresentationResult.rewardVerificationFailed)

        let failed = RewardedInterstitialPresentation.Outcome.failed(NSError(domain: "gma", code: 1)).presentationResult
    ...
    private func makePresentation(
        loadResult: Result<FakeRewardedInterstitialAd, Error>,
        ...
        return RewardedInterstitialPresentation(
    ...
    private func start(
        _ presentation: RewardedInterstitialPresentation,
        placement: String = "checkpoint",
        completion: @escaping @MainActor (RewardedInterstitialPresentation.Outcome) -> Void
    ) {
        presentation.start(
            adUnitID: Self.adUnitID,
            mediator: .adMob,
    ...
    private func waitForPresentation(of fakeAd: FakeRewardedInterstitialAd) {
...
private final class OutcomeRecorder {
    ...
    private(set) var outcomes: [RewardedInterstitialPresentation.Outcome] = []

    func record(_ outcome: RewardedInterstitialPresentation.Outcome) {
...
@available(iOS 15.0, *)
private extension RewardedInterstitialPresentation.Outcome {

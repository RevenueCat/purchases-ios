...
Component field names are generated from serialized override `properties` JSON keys instead of a fixed SDK-authored list. Direct properties map to `component.<serialized_property_name>`, and reducers can compose multiple properties into derived fields in later component slices. Use each component's `CodingKeys` string values rather than manually normalizing names.
...
- Deliver committed events through a serialized mechanism so concurrent async gate resolutions cannot call Combine `send` concurrently.
...
Existing Paywalls V2 interaction surfaces serve as the compatibility replacement for POC-style controls and effects:

| POC concept | Paywalls V2 first-slice mapping |
| --- | --- |
| `controls` writing arbitrary state IDs | Existing package row selection and tab package selection request package-selection mutations |
| `effects.visibility` | Existing override `visible` properties project to `component.visible` fields |
| Animation/displacement metadata | Not supported in the first slice without new JSON fields |
| Arbitrary server-defined state slots | Not introduced in the first slice; keys are generated from existing override properties and known interaction surfaces |
...
The adapter should register expected value kinds for generated fields. Direct raw-property fields can accept `PaywallStateValue.json(...)`, while SDK-known rendered fields such as package IDs or processed text can use typed values. Gate replacements that target an unregistered field or provide an incompatible value kind should be rejected with diagnostics instead of silently committing invalid state.
...
3. Produce projected field values from raw property keys: for example `visible` -> `component.visible` and `text` -> `component.text`.
...
Add the state store, state keys, scoped identities, mutation proposal, and event pipeline. `PaywallsV2View` should create a store when no ancestor store is injected, but it must also support an injected shared store so multiple paywall scopes can coexist in the same pipeline. No component behavior changes yet.
...
Add projected style fields and committed-state observers for `TextComponentView`. Start with:
...
The legacy `TextComponentViewModel.styles(...)` path remains available as initialization/fallback while committed field state is absent.
...
- tab-driven package changes go through the same proposed mutation pipeline as package row taps

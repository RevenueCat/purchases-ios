...
Component field names are generated from serialized override `properties` JSON keys instead of a fixed SDK-authored list. Direct properties map to `component.<serialized_property_name>`, and reducers can compose multiple properties into derived fields. Use each component's `CodingKeys` string values rather than manually normalizing names. For example, an icon override containing color, formats, and icon-name properties emits direct fields for those serialized property names and can also emit a derived icon URL field from formats + icon name.
...
- an icon URL field derived from the icon-name and formats coding keys
...
3. Produce projected field values from raw property keys: for example `visible` -> `component.visible`, `text` -> `component.text`, and reducer-derived fields like icon-name + formats coding keys -> an icon URL field.

...
extension ProductsManager: SKProductsRequestDelegate {
    ...
    func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        queue.async { [self] in
            ...
            guard let completionBlocks = self.completionHandlers[requestProducts] else { fatalError("couldn't find completion") }
    ...
    func request(_ request: SKRequest, didFailWithError error: Error) {
        queue.async { [self] in
            ...
            guard let completionBlocks = self.completionHandlers[products] else { fatalError("couldn't find completion") }

...
/**
 Completion block for `-[Purchases checkTrialOrIntroductoryPriceEligibility:completionBlock:]`
 */
public typealias ReceiveIntroEligibilityBlock = ([String: IntroEligibility]) -> Void

/**
 Completion block for `-[Purchases offeringsWithCompletionBlock:]`
 */

public typealias ReceiveOfferingsBlock = (Offerings?, Error?) -> Void

/**
 Completion block for `-[Purchases productsWithIdentifiers:completionBlock:]`
 */
...
/**
 * Deferred block for `-[Purchases paymentDiscountForProductDiscount:product:completion:]`
 */
...
public extension Purchases {
     ...
     * the user. Typically with a button in settings or near your purchase UI. Use `syncPurchases(completionBlock:)`
     ...
     * `IntroEligibilityStatusUnknown`. The best course of action on unknown status is to display the non-intro

...
class BackendTests: XCTestCase {
    ...
    let validEmptySubscriberResponse : [String:Any] = [
        "request_date": "2019-07-27T00:02:22Z",
        "request_date_ms": 1564185742917,
        "subscriber": [
            "entitlements": [:],
    ...
    let validRichSubscriberResponse : [String:Any] = [
        "request_date": "2019-07-27T00:02:22Z",
        "request_date_ms": 1564185742917,
        "subscriber": [
            "entitlements": [
    ...
    func testGetsEmptySubscriberInfo() {
        let response = HTTPResponse(statusCode: 200, response: validEmptySubscriberResponse, error: nil)
        httpClient.mock(requestPath: "/subscribers/" + userID, response: response)
        ...
        expect(subscriberInfo).toEventuallyNot(beNil())
        expect(subscriberInfo?.firstSeen).toEventuallyNot(beNil())
        expect(subscriberInfo?.originalAppUserId).toEventually(equal("cesarsandbox1"))
        expect(subscriberInfo?.entitlements.all.count).toEventually(be(0))
    }

    func testCreatesEntitlementInfos() {
        let response = HTTPResponse(statusCode: 200, response: validRichSubscriberResponse, error: nil)
        httpClient.mock(requestPath: "/subscribers/" + userID, response: response)
        ...
        expect(subscriberInfo).toEventuallyNot(beNil())
        expect(subscriberInfo?.entitlements).toEventuallyNot(beNil())
        expect(subscriberInfo?.originalAppUserId).toEventually(equal("cesarsandbox1"))
        expect(subscriberInfo?.entitlements.all.count).toEventually(be(1))
        expect(subscriberInfo?.entitlements.all.keys.contains("pro_cat")).toEventually(beTrue())
        expect(subscriberInfo?.entitlements.active.keys.contains("pro_cat")).toEventually(beTrue())
    }

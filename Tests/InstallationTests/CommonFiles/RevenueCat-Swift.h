...
/// returns:
/// An instantiated <code>Purchases</code> object that has been set as a singleton.
+ (RCPurchases * _Nonnull)configureWithAPIKey:(NSString * _Nonnull)apiKey appUserID:(NSString * _Nullable)appUserID observerMode:(BOOL)observerMode SWIFT_AVAILABILITY(macos,obsoleted=1,message="'configure' has been renamed to 'configureWithAPIKey:appUserID:observerMode:storeKitVersion:': Explicitly setting the StoreKit version is now required when using observer mode.") SWIFT_AVAILABILITY(watchos,obsoleted=1,message="'configure' has been renamed to 'configureWithAPIKey:appUserID:observerMode:storeKitVersion:': Explicitly setting the StoreKit version is now required when using observer mode.") SWIFT_AVAILABILITY(tvos,obsoleted=1,message="'configure' has been renamed to 'configure(withAPIKey:appUserID:storeKitVersion:)': Explicitly setting the StoreKit version is now required when using observer mode.") SWIFT_AVAILABILITY(ios,obsoleted=1,message="'configure' has been renamed to 'configureWithAPIKey:appUserID:observerMode:storeKitVersion:': Explicitly setting the StoreKit version is now required when using observer mode.");
@end

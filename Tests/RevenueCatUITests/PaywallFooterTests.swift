...
class PaywallFooterTests: TestCase {
    ...
    // The deployed backend wraps every offering in a workflow, so the mock must too or the footer
    // paywall never renders (and these purchase/restore callbacks never fire).

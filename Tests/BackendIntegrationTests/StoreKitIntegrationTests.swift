...
class StoreKit2IntegrationTests: StoreKit1IntegrationTests {
    ...
    func testOriginalPurchaseDateAvailableAfterSyncPurchasesWithPreviousPurchase() async throws {
        ...
        // called
        guard (try await self.monthlyPackage.storeProduct.sk2Product) != nil else {
...
class StoreKit1IntegrationTests: BaseStoreKitIntegrationTests {
    ...
    func testRenewalsOnASeparateUserDontTransferPurchases() async throws {
        ...
        let purchaseResult = try await self.purchaseMonthlyOffering()

        // Get the transaction ID for proper control
        ...
        try self.testSession.forceRenewalOfSubscription(productIdentifier: transaction.productIdentifier)
    ...
    func testUserCanMakePurchaseAfterTransferBlocked() async throws {
        ...
        // Get the transaction for proper control
        let transaction = try XCTUnwrap(purchaseResult.transaction)
        ...
        // 3. Force renewal instead of using time manipulation
        self.logger.clearMessages()
    ...
    func testIneligibleForIntroAfterPurchase() async throws {
        ...
        let purchaseResult = try await self.purchaseShortestDuration()

        let transaction = try XCTUnwrap(purchaseResult.transaction)
        try self.testSession.expireSubscription(productIdentifier: transaction.productIdentifier)
    ...
    func testIneligibleForIntroAfterPurchaseExpires() async throws {
        ...
        // 1. Purchase shortest duration product
        let customerInfo = try await self.purchaseShortestDuration().customerInfo

        // 2. Get entitlement and expire using native method
        let entitlement = try XCTUnwrap(customerInfo.entitlements[Self.entitlementIdentifier])
        try await self.expireSubscription(entitlement)
    ...
    func testSubscribeAfterExpirationWhileAppIsClosed() async throws {
        ...
        // 3. Expire subscription using native method instead of time manipulation
        try await self.expireSubscription(entitlement)

        // 4. Re-open app
        await self.resetSingleton()

        // 5. Wait for pending transactions to be posted
    ...
    func testUserHasNoEligibleOffersByDefault() async throws {
        ...
        let offers = try await self.purchases.eligiblePromotionalOffers(forProduct: product)
    ...
    func testPurchaseWithPromotionalOffer() async throws {
        ...
        let offer = try await XCTAsyncUnwrap(
            await self.purchases
                .eligiblePromotionalOffers(forProduct: product)
                .onlyElement
        )

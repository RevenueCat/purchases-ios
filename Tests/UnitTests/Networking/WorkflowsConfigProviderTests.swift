...
class WorkflowsConfigProviderTests: TestCase {
    ...
    func testResolvesAWorkflowAlreadyCommittedToTheWorkflowsTopic() async throws {
        ...
        // No `combo_key` means the workflow has no experiment.
        expect(workflowResult.enrolledVariants).to(beNil())
    }

    func testParsesEnrolledVariantsFromTheItemComboKey() async throws {
        self.commit(
            workflows: ["wf-1": .init(
                blobRef: "wf-1-ref",
                content: ["combo_key": "exp_b=control;exp_a=variant_2"]
            )],
        ...
        let workflowResult = try XCTUnwrap(result.value)
        expect(workflowResult.enrolledVariants) == ["exp_a": "variant_2", "exp_b": "control"]
    }

    func testEnrolledVariantsAreNilForTheDefaultComboKey() async throws {
        self.commit(
            workflows: ["wf-1": .init(blobRef: "wf-1-ref", content: ["combo_key": "default"])],
            uiConfig: Self.uiConfigTopic,
            blobs: Self.uiConfigBlobs.merging(["wf-1-ref": try Self.workflowJSON(id: "wf-1")]) { current, _ in current }
        )
    ...
    func testEnrolledVariantsSkipMalformedComboKeyPairs() async throws {
        self.commit(
            workflows: ["wf-1": .init(
                blobRef: "wf-1-ref",
    ...
    func testEnrolledVariantsAreNilWhenEveryComboKeyPairIsMalformed() async throws {
        self.commit(
            workflows: ["wf-1": .init(blobRef: "wf-1-ref", content: ["combo_key": "garbage"])],
            uiConfig: Self.uiConfigTopic,
            blobs: Self.uiConfigBlobs.merging(["wf-1-ref": try Self.workflowJSON(id: "wf-1")]) { current, _ in current }
        )
    ...
    func testParsesEnrolledVariantsFromARealWireDecodedTopic() async throws {
        let topicsJSON = """
        {
          ...
          "ui_config": {
            ...
            "localizations": { "blob_ref": "loc-ref" },
            "variable_config": { "blob_ref": "variable-config-ref" },
            "custom_variables": { "blob_ref": "custom-variables-ref" }
          }
        }
        """
        ...
        self.diskCache.stubbedRead = PersistedRemoteConfiguration(
            ...
            activeTopics: ["workflows", "ui_config"],
            topics: topics
        )
        self.blobStore.stubbedData = Self.uiConfigBlobs.merging(
            ["wf-1-ref": try Self.workflowJSON(id: "wf-1")]
    ...
    func testCachedWorkflowsCarryTheirEnrolledVariants() async throws {
        self.commit(
            workflows: [
                "wf-prefetch": .init(
                    blobRef: "wf-prefetch-ref",
                    prefetch: true,
                    content: ["offering_identifier": "premium", "combo_key": "exp_a=b"]
                ),
                "wf-current": .init(
                    blobRef: "wf-current-ref",
                    content: ["offering_identifier": "basic", "combo_key": "default"]
                )
            ],
            uiConfig: Self.uiConfigTopic,
            blobs: Self.uiConfigBlobs.merging([
                "wf-prefetch-ref": try Self.workflowJSON(id: "wf-prefetch"),
                "wf-current-ref": try Self.workflowJSON(id: "wf-current")
            ]) { current, _ in current }
        )
        ...
        let readPrefetched = await self.provider.getWorkflow(workflowId: "wf-prefetch")
        let prewarm = await self.provider.decodeCachedWorkflowForAssetPrewarming(workflowId: "wf-prefetch")

        expect(cachedPrefetched?.enrolledVariants) == ["exp_a": "b"]
        expect(cachedCurrent?.workflow.id) == "wf-current"
        expect(cachedCurrent?.enrolledVariants).to(beNil())
        expect(readPrefetched.value?.enrolledVariants) == ["exp_a": "b"]
        expect(prewarm.value?.enrolledVariants) == ["exp_a": "b"]
    }
